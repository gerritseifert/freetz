...
$(PKG)_BINS_NETATALKD := netatalk
$(PKG)_BINS_NETATALKD_BUILD_DIR := $($(PKG)_BINS_NETATALKD:%=$($(PKG)_DIR)/etc/netatalk/.libs/%)
$(PKG)_BINS_NETATALKD_TARGET_DIR := $($(PKG)_BINS_NETATALKD:%=$($(PKG)_DEST_DIR)/sbin/%)
...
$($(PKG)_BINS_NETATALKD_TARGET_DIR): $($(PKG)_DEST_DIR)/sbin/%: $($(PKG)_DIR)/etc/netatalk/.libs/%
...
$(pkg)-precompiled: $($(PKG)_LIBS_TARGET_DIR) $($(PKG)_ATALK_TARGET_BINARY) $($(PKG)_BINS_NETATALKD_TARGET_DIR) $($(PKG)_BINS_AFPD_TARGET_DIR) $($(PKG)_BINS_DBD_TARGET_DIR)

$(pkg)-clean:
	-$(SUBMAKE) -C $(NETATALK_DIR) clean

$(pkg)-uninstall:
	$(RM) $(NETATALK_LIBS_TARGET_DIR) $(NETATALK_BINS_NETATALKD_TARGET_DIR) $(NETATALK_BINS_AFPD_TARGET_DIR) $(NETATALK_BINS_DBD_TARGET_DIR)
